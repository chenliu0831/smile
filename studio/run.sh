...
# AWS_BEARER_TOKEN_BEDROCK); the Tauri app passes its environment to the spawned JVM. When
# you run this script from your terminal, that env var is already inherited — we just check
# it's present and warn (NON-fatally) if not. If it's missing the app still launches and
# shows a "No daemon" badge; you don't need a token just to open the window.
#
# NOTE: we deliberately do NOT source ~/.zshrc — sourcing a zsh profile from bash is
# unreliable (interactive-only guards / zsh syntax can abort the script). Just run this
# from a shell where the var is exported, or `export AWS_BEARER_TOKEN_BEDROCK=...` first.
if [[ -n "${AWS_BEARER_TOKEN_BEDROCK:-}" ]]; then
  echo "▸ LLM credential detected (AWS_BEARER_TOKEN_BEDROCK) — daemon can start."
else
  echo "⚠  AWS_BEARER_TOKEN_BEDROCK is not set in this shell. The app will still open, but"
  echo "   in 'No daemon' mode (can't analyze real data). To enable analysis, run:"
  echo "       export AWS_BEARER_TOKEN_BEDROCK=…   # then re-run ./studio/run.sh"
  echo "   (or set OPENAI_API_KEY / GOOGLE_API_KEY / ANTHROPIC_API_KEY for another provider)."
fi
